...
export function registerGuard(router: Router) {
    router.beforeEach(async (to: RouteLocationNormalized, from: RouteLocationNormalized) => {
        if (to.meta.requiresAuth) {
            ...
            const shouldProceed = await isAuthenticated(msalInstance, InteractionType.Redirect, request, true);
...
export async function isAuthenticated(instance: PublicClientApplication, interactionType: InteractionType, loginRequest: PopupRequest|RedirectRequest, checkAdmin: boolean = false): Promise<boolean> {    
        ...
        if (accounts.length > 0) {
            if(checkAdmin) {
                ...
                if(user && user.idTokenClaims && user.idTokenClaims.roles) {
                    return user.idTokenClaims.roles?.includes("chat.admin");
                }
                return false;
            }
            return true;
